...
var signal_table: Array = [
	...
	{"singleton": "NetworkManager", "signal": "entity_network_id_registered", "method": "_network_id_registered_added"},
	{
		...
		"signal": "entity_network_id_unregistered",
		"method": "_network_id_unregistered_added"
	},
...
func get_network_scene_id_from_path(p_path: String) -> int:
	...
	while 1:
		...
		if network_scene_id == -1:
			if (
				path != "res://addons/vsk_entities/vsk_player.tscn"
				and path != "res://addons/vsk_entities/vsk_interactable_prop.tscn"
				and path != "res://addons/vsk_entities/vsk_test_entity.tscn"
			):
				push_error(
					"SECURITY: " + str(self) + "@" + str(self.get_path()) + ": Checking for resource at " + str(path)
				)
				break
...
func _network_manager_process(p_id: int, _delta: float) -> void:
	if (
		network_entity_ids_pending_spawn.size() > 0
		or network_entity_ids_pending_destruction.size()
		or network_entity_ids_pending_request_transfer_master.size() > 0
	):
		...
		if network_entity_ids_pending_spawn.size():
			...
			for entity_id in network_entity_ids_pending_spawn:
				var entity_instance: Node = network_manager.network_entity_manager.get_network_instance_for_instance_id(
					entity_id
				)
		...
		for synced_peer in synced_peers:
			...
			if p_id == network_manager.session_master:
				...
				for entity_id in network_entity_ids_pending_spawn:
					...
					if ignore_list.has(
						network_manager.network_entity_manager.get_network_instance_for_instance_id(entity_id)
					):
						continue
...
func get_packed_scene_for_path(p_path: String) -> PackedScene:
	if (
		p_path != "res://addons/vsk_entities/vsk_player.tscn"
		and p_path != "res://addons/vsk_entities/vsk_interactable_prop.tscn"
		and p_path != "res://addons/vsk_entities/vsk_test_entity.tscn"
	):
		push_error(
			"SECURITY: " + str(self) + "@" + str(self.get_path()) + ": get_packed_scene_for_path at " + str(p_path)
		)
		return null
...
func decode_entity_spawn_command(p_packet_sender_id: int, p_network_reader: Object) -> Object:
	...
	if scene_path == "":
		NetworkLogger.error(
			"decode_entity_spawn_command: received invalid scene id {scene_id}!".format({"scene_id": scene_id})
		)
		return null
	...
	_EntityManager.scene_tree_execution_command(
		_EntityManager.scene_tree_execution_table_const.ADD_ENTITY, entity_instance
	)
...
func decode_entity_destroy_command(p_packet_sender_id: int, p_network_reader: Object) -> Object:
	...
	if network_entity_manager.network_instance_ids.has(instance_id):
		...
		_EntityManager.scene_tree_execution_command(
			_EntityManager.scene_tree_execution_table_const.REMOVE_ENTITY, entity_instance
		)
	else:
